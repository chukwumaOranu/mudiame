...
const MenuItemes2 = [
  {
    ...
    sub_menu: [
      ...
      {
        sub_menu_item: "Detailed Grid Sidebar",
        link: "/detailed-grid-sidebar",
      },
...
const Menu = () => {
  ...
  window.onscroll = function () {
    ...
    scrollValue > 80 ? setHeader(true) : setHeader(false);
  };
  useEffect(() => {
    const getPath = PathName.pathname;
    MenuItemes1.map((item) => {
      item.sub_menu?.map((item2) => {
        if (item2.link === getPath) {
          setPath(item.MenuLinks);
        }
      });
    });
    MenuItemes2.map((item) => {
      item.sub_menu.map((item2) => {
        if (item2.link === getPath) {
          setPath(item.MenuLinks);
        }
        ...
        <div className="container clearfix">
          ...
          >
            ...
            <ul className="nav navbar-nav">
              {MenuItemes1.map((item, index) => {
                const Class = item.class;
                if (Class === "sub-menu-down") {
                  return (
                    <li
                      className={`
                                                ${
                  ...
                  return (
                    <li
                      className={`${path === item.MenuLinks ? "active" : ""}`}
                    >
                      <Link to={`#`}>{item.MenuLinks}</Link>
                    </li>
                  );
                }
              })}
